...
import { Search, ChevronLeft, ChevronRight, UserCheck, ShieldAlert } from 'lucide-react'
...
export const PlanningPage: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [sumRes, detRes] = await Promise.all([
        api.get(`/planning/summary?tahun=${tahun}&triwulan=${triwulan}`),
        api.get(`/planning/detail?tahun=${tahun}&triwulan=${triwulan}${selectedDept ? `&departemen=${encodeURIComponent(selectedDept)}` : ''}${search ? `&search=${encodeURIComponent(search)}` : ''}`),
      ])
  ...
  const getStatusBadge = (status: string) => {
    ...
    if (s.includes('wait')) return <Badge variant="warning">Waiting Approval</Badge>
    if (s.includes('draft')) return <Badge variant="info">Drafted</Badge>
  ...
  return (
    <div className="space-y-6">
      {/* Upload Slot */}
      <UploadPanel currentModule="kpi_planning" moduleLabel="Report KPI Planning" />

      {/* Action Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h3 className="text-xl font-bold text-text-primary">Performance Planning Overview</h3>
          <p className="text-xs text-text-muted">
            Status perencanaan KPI perorangan & unit kerja (Tahun {tahun} TW {triwulan})
          </p>
        </div>
        <DownloadReportButton modul="planning" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="border-border-custom bg-bg-card flex flex-col justify-between">
          <CardHeader className="p-4 border-b border-border-custom/50">
            <CardTitle className="text-sm font-bold">Overall Planning Status</CardTitle>
            <CardDescription className="text-xs">Distribusi status KPI karyawan</CardDescription>
          </CardHeader>
          ...
          <CardHeader className="pb-3 border-b border-border-custom/50">
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription className="text-xs">Progress penyelesaian KPI per unit kerja</CardDescription>
              </div>
              {selectedDept && (
                <button
      ...
      <Card className="border-border-custom bg-bg-card">
        <CardHeader>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <CardTitle className="text-sm font-bold">Detail Perencanaan KPI Karyawan</CardTitle>
                  ...
                  <TableRow key={idx}>
                    ...
                    <TableCell>
                      <div className="font-semibold text-text-primary flex items-center gap-1.5">
                        ...
                        {emp.is_delegasi && (
                          <span
                            title={`Delegasi Posisi Lain: ${emp.delegasi_posisi_lain || '-'}`}
                    ...
                    <TableCell className="text-xs text-text-muted">{emp.submit_date || '-'}</TableCell>
                    <TableCell className="text-xs text-text-muted">{emp.approved_date || '-'}</TableCell>
                  </TableRow>
                ))
              ) : (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-8 text-text-muted text-xs">
                    {isLoading ? 'Memuat data...' : 'Tidak ada catatan karyawan ditemukan.'}
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
